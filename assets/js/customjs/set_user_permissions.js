...
function load_user_select() {
    $.ajax({
        url: window.location.origin + '/user_controller/get_user_for_set_permissions',
        dataType: 'json',
        type: 'post',
        success: function(params) {
            if (params !== false) {
                ...
                Array.from(params.userData).map(row => {
                    html = html + '<option value="' + row.userid + '">' + row.userName + ' ( ' + row.userLocation + ' ) ' + '</option>';
                });
...
function load_menu_table(id) {
    $('#menu_table').DataTable({
        "destroy": true,
        "ajax": {
            "url": window.location.origin + "/user_controller/get_user_menu_table",
            "type": "POST",
            "data": { userid: id }
        },
        "columns": [
...
function load_permission_table(userid,subid) {
    $('#permission_table').DataTable({
        "destroy": true,
        "ajax": {
            "url": window.location.origin + "/user_controller/get_user_permission_table",
            "type": "POST",
...
$(document).on('change', '[name="submenu"]', function() {
    ...
    if (checkbox.is(':checked')) {
        $.ajax({
            url: window.location.origin + '/user_controller/insert_menu',
            type: 'post',
            dataType: 'json',
        ...
        $.ajax({
            url: window.location.origin + '/user_controller/remove_menu',
            type: 'post',
            dataType: 'json',
            data: { userid: $user, subid: $submenu },
            success: function(params) {
...
$(document).on('change', '[name="permission"]', function() {
    ...
    if (checkbox.is(':checked')) {
        $.ajax({
            url: window.location.origin + '/user_controller/set_permission',
            type: 'post',
            dataType: 'json',
        ...
        $.ajax({
            url: window.location.origin + '/user_controller/remove_permission',
            type: 'post',
            dataType: 'json',
            data: { userid: $user, permid: $permission },
            success: function(params) {
